...
if ($argc < 2) {
    fwrite(STDERR, "Usage: php scripts/import_gba_roms.php <source-folder> [collection-slug]\n");
    exit(1);
}

$sourceRoot = $argv[1];
$collectionSlug = $argv[2] ?? '_gba_nintendo_pack';
...
$targetRoot = $siteRoot . DIRECTORY_SEPARATOR . 'gba' . DIRECTORY_SEPARATOR . 'games' . DIRECTORY_SEPARATOR . $collectionSlug;
$imageUrl = '/common-file/img/default-gba.svg';
...
function display_name_from_gba_file(string $filename): string
{
...
function bucket_key_for_gba_name(string $name): string
{
...
function gba_url_from_relative_path(string $relativePath): string
{
    $segments = explode('/', str_replace('\\', '/', $relativePath));
    return implode('/', array_map('rawurlencode', $segments));
}

function unique_gba_target_name(string $bucketDir, string $basename, string $relativeSource): string
{
...
function ensure_gba_category(PDO $pdo, string $name): int
{
...
function link_gba_game_category(PDO $pdo, int $gameId, int $categoryId): void
{
...
function backup_gba_catalog(PDO $pdo, string $siteRoot, string $collectionSlug): string
{
    $backupName = trim($collectionSlug, '_-');
    $backupName = $backupName !== '' ? $backupName : 'gba';
    $backupPath = (getenv('NANDAR_IMPORT_BACKUP_DIR') ?: 'C:\\laragon\\private\\import_backups') . DIRECTORY_SEPARATOR . 'gba_import_' . $backupName . '_backup_' . date('Ymd_His') . '.json';
    $payload = [
...
function ensure_gba_category_lookup_index(PDO $pdo): void
{
    $stmt = $pdo->prepare(
        "SELECT COUNT(*)
...
foreach ($iterator as $fileInfo) {
    ...
    if (strtolower($fileInfo->getExtension()) !== 'gba') {
        continue;
    }

    $fullPath = $fileInfo->getPathname();
    $relativeSource = ltrim(substr($fullPath, strlen($sourceRoot)), "\\/");
    $displayName = display_name_from_gba_file($fileInfo->getFilename());
    $bucket = bucket_key_for_gba_name($displayName);
...
$backupPath = backup_gba_catalog($pdo, $siteRoot, $collectionSlug);
ensure_gba_category_lookup_index($pdo);
...
try {
    ...
    $selectExistingCollection->execute([
        ':prefix' => '/gba/play.html?file=' . rawurlencode($collectionSlug) . '/%',
    ]);
    ...
    foreach (array_merge(['0-9'], range('A', 'Z'), ['Other']) as $bucket) {
        $categoryIds[$bucket] = ensure_gba_category($pdo, 'GBA ' . $bucket);
    }
...
echo "Playable GBA ROMs found: " . count($roms) . "\n";
