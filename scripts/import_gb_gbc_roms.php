...
if ($argc < 2) {
    fwrite(STDERR, "Usage: php scripts/import_gb_gbc_roms.php <source-folder> [collection-slug]\n");
    exit(1);
}

$sourceRoot = $argv[1];
$collectionSlug = $argv[2] ?? '_gbgbc_from_gba_pack';
...
$targetRoot = $siteRoot . DIRECTORY_SEPARATOR . 'gb' . DIRECTORY_SEPARATOR . 'games' . DIRECTORY_SEPARATOR . $collectionSlug;
$imageUrl = '/common-file/img/default-gb.svg';
$parentCategoryName = 'GB / GBC';
...
function display_name_from_gb_file(string $filename): string
{
...
function bucket_key_for_gb_name(string $name): string
{
...
function gb_url_from_relative_path(string $relativePath): string
{
    $segments = explode('/', str_replace('\\', '/', $relativePath));
    return implode('/', array_map('rawurlencode', $segments));
}

function unique_gb_target_name(string $bucketDir, string $basename, string $relativeSource): string
{
...
function ensure_gb_category(PDO $pdo, string $name): int
{
...
function link_gb_game_category(PDO $pdo, int $gameId, int $categoryId): void
{
...
function backup_gb_catalog(PDO $pdo, string $siteRoot, string $collectionSlug): string
{
    $backupName = trim($collectionSlug, '_-');
    $backupName = $backupName !== '' ? $backupName : 'gbgbc';
    $backupPath = (getenv('NANDAR_IMPORT_BACKUP_DIR') ?: 'C:\\laragon\\private\\import_backups') . DIRECTORY_SEPARATOR . 'gbgbc_import_' . $backupName . '_backup_' . date('Ymd_His') . '.json';
    $payload = [
...
function ensure_gb_indexes(PDO $pdo): void
{
    $categoryIndex = $pdo->prepare(
        "SELECT COUNT(*)
    ...
    $categoryIndex->execute();

    if ((int) $categoryIndex->fetchColumn() === 0) {
        $pdo->exec('ALTER TABLE game_category ADD INDEX idx_category_game (category_id, game_id)');
    }

    $gameUrlIndex = $pdo->prepare(
        "SELECT COUNT(*)
         FROM information_schema.statistics
         WHERE table_schema = DATABASE()
           AND table_name = 'games'
           AND index_name = 'idx_games_game_url'"
    );
    $gameUrlIndex->execute();

    if ((int) $gameUrlIndex->fetchColumn() === 0) {
        $pdo->exec('ALTER TABLE games ADD INDEX idx_games_game_url (game_url(255))');
    }
}

$roms = [];
$allowedExtensions = ['gb' => true, 'gbc' => true];
...
foreach ($iterator as $fileInfo) {
    ...
    $displayName = display_name_from_gb_file($fileInfo->getFilename());
    $bucket = bucket_key_for_gb_name($displayName);
...
$backupPath = backup_gb_catalog($pdo, $siteRoot, $collectionSlug);
ensure_gb_indexes($pdo);
...
try {
    ...
    $selectExistingCollection->execute([
        ':prefix' => '/gb/play.html?file=' . rawurlencode($collectionSlug) . '/%',
    ]);
    foreach ($selectExistingCollection->fetchAll() as $row) {
        $existingGameUrl[(string) $row['game_url']] = true;
    }

    $parentCategoryId = ensure_gb_category($pdo, $parentCategoryName);
    $categoryIds = [];
    foreach (array_merge(['0-9'], range('A', 'Z'), ['Other']) as $bucket) {
        $categoryIds[$bucket] = ensure_gb_category($pdo, 'GB/GBC ' . $bucket);
    }
    ...
    foreach ($roms as $rom) {
        ...
        $targetName = unique_gb_target_name($bucketDir, $rom['filename'], $rom['relative_source']);
        $targetPath = $bucketDir . DIRECTORY_SEPARATOR . $targetName;
        $relativeGameFile = $collectionSlug . '/' . $rom['bucket'] . '/' . $targetName;
        $gameUrl = '/gb/play.html?file=' . gb_url_from_relative_path($relativeGameFile);

        if (isset($existingGameUrl[$gameUrl])) {
            $selectGame->execute([':game_url' => $gameUrl]);
            $gameId = (int) $selectGame->fetchColumn();
            if ($gameId > 0) {
                link_gb_game_category($pdo, $gameId, $parentCategoryId);
                link_gb_game_category($pdo, $gameId, $categoryIds[$rom['bucket']]);
            }
            $skippedExisting++;
            continue;
        }
...
echo "Playable GB/GBC ROMs found: " . count($roms) . "\n";
