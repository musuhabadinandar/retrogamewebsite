class EJS_GameManager {
    ...
    getRetroArchCfg() {
        const rawAudioLatency = this.EJS.config ? this.EJS.config.audioLatency : null;
        const parsedAudioLatency = Number(rawAudioLatency);
        const audioLatency = rawAudioLatency === null || rawAudioLatency === undefined || rawAudioLatency === "" || !Number.isFinite(parsedAudioLatency)
            ? 64
            : Math.max(8, Math.min(512, Math.round(parsedAudioLatency)));
